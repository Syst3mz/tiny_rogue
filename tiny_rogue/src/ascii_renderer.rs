...
pub struct AsciiRenderer {
    buffer: [char; DRAWING_BUFFER_SIZE],
...
impl AsciiRenderer {
    fn new(fill: char, character_metrics: Vector2<usize>) -> Self {
        Self {
            buffer: [fill; DRAWING_BUFFER_SIZE],
            character_metrics,
        }
    }
    ...
    pub fn row(&self, row: usize) -> impl Iterator<Item=char>{
        let row_start = row * SCREEN_SIZE.x;
        (row_start..row_start + SCREEN_SIZE.x).map(|x| self.buffer[x])
    }
}
...
impl Renderer for AsciiRenderer {
    ...
    fn clear(&mut self, with: Option<char>) {
        ...
        self.buffer = [with; DRAWING_BUFFER_SIZE];
    }
